...
function defaultExcludeFields(model, exclude) {
  return Object.keys(model.rawAttributes).reduce(function (data, current) {
    var item = model.rawAttributes[current];
...
function argsToSequelize(model, args) {
  return Object.keys(args).reduce(function (data, current) {
    if (model.rawAttributes[current]) {
      data[current] = model.rawAttributes[current].primaryKey || model.rawAttributes[current].references ? parseInt((0, _graphqlRelay.fromGlobalId)(args[current]).id, 10) : args[current];
    }
