...
function resolve(target, source, args, context, info) {
  ...
  var cursor = (0, _cursor.fromCursor)(after || before);
  ...
  return source["get" + fnTargetname + "s"](options).then(function (items) {
    var edges = items.map(function (item) {
      return { node: item, cursor: (0, _cursor.toCursor)(item) };
    });
    ...
    var prevOptions = Object.assign({}, options, {
      where: {
        "createdAt": {
          $lt: Number((0, _cursor.fromCursor)(result.pageInfo.startCursor))
        }
      }
    });
    ...
    var nextOptions = Object.assign({}, options, {
      where: {
        "createdAt": {
          $gt: Number((0, _cursor.fromCursor)(result.pageInfo.endCursor))
        }
      }
    });
